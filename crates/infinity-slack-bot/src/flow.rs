...
pub fn slack_dataflow<'a, P: 'a>(
    ...
    let command_response_actions =
        slash_commands.filter_map(q!(|event: crate::sidecar::SlackEvent| {
            ...
            match command.as_str() {
                "/model" => {
                    ...
                    if arg.is_empty() {
                        if let Some(trigger_id) = event.trigger_id {
                            ...
                            {
                                return Some(crate::sidecar::SlackAction::OpenView {
                                    trigger_id,
                                    view,
                                });
                            }
                ...
                other => Some(crate::sidecar::SlackAction::CommandResponse {
                    response_url,
    ...
    let daemon_slack_actions =
        daemon_events.flat_map_ordered(q!(|de: crate::daemon_sidecar::DaemonEvent| {
            let rt = crate::runtime::get();
            let channel = rt
                .channels
                .lock()
            ...
            match &de.message {
                infinity_protocol::DaemonMessage::Connected {
                    ...
                    vec![crate::sidecar::SlackAction::SetThreadTitle {
                ...
                infinity_protocol::DaemonMessage::SessionsUpdated { sessions } => {
                    ...
                    let Some(title) = sessions.get(&session_id).and_then(|s| s.title.clone())
                    else {
                        return Vec::new();
                    };
                    let mut titles = rt.thread_titles.lock().expect("bug: lock poisoned");
                ...
                infinity_protocol::DaemonMessage::TextChunk { chunk, .. } => {
                    ...
                    if has_thread {
                        actions.push(crate::sidecar::SlackAction::SetStatus {
                            channel: channel.clone(),
                            thread_ts: de.thread_ts.clone(),
                            status: "is thinking".to_owned(),
                        });
                    }
                    actions.push(crate::sidecar::SlackAction::StreamAppend {
                        channel: channel.clone(),
                        thread_ts: de.thread_ts,
                        text: chunk.clone(),
                    });
                ...
                infinity_protocol::DaemonMessage::ToolCall { name, args, .. } => {
                    ...
                    actions.push(crate::sidecar::SlackAction::StreamTaskUpdate {
                        channel: channel.clone(),
                        thread_ts: de.thread_ts,
                        task_id,
                        title: name.clone(),
                ...
                infinity_protocol::DaemonMessage::ResponseDone { .. } => {
                    ...
                    if had_tool {
                        ...
                        vec![crate::sidecar::SlackAction::StreamStop {
                            channel: channel.clone(),
                            thread_ts: de.thread_ts,
                        }]
                    }
                ...
                infinity_protocol::DaemonMessage::Error { text, .. } => {
                    vec![crate::sidecar::SlackAction::PostMessage {
                        channel: channel.clone(),
                        text: format!("⚠️ {text}"),
                ...
                infinity_protocol::DaemonMessage::UserChoiceRequired {
                    ...
                    vec![
                        ...
                        crate::sidecar::SlackAction::PostBlocks {
                            channel: channel.clone(),
                ...
                infinity_protocol::DaemonMessage::UserChoiceComplete { choice_id } => {
                    ...
                    vec![crate::sidecar::SlackAction::DismissChoiceButtons {
...
pub fn build_model_picker_view(response_url: &str) -> Option<serde_json::Value> {
    ...
    let mut element = serde_json::json!({
        "type": "static_select",
        "action_id": crate::sidecar::MODEL_PICKER_ACTION_ID,
    ...
    Some(serde_json::json!({
        "type": "modal",
        "callback_id": crate::sidecar::MODEL_PICKER_CALLBACK_ID,
        ...
        "blocks": [
            {
                "type": "input",
                "block_id": crate::sidecar::MODEL_PICKER_BLOCK_ID,
...
mod tests {
    ...
    fn model_picker_view_reflects_available_models() {
        ...
        assert_eq!(
            view["callback_id"],
            crate::sidecar::MODEL_PICKER_CALLBACK_ID
        );
        assert_eq!(
