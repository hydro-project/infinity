...
//!
//! These look for eaten/overwritten scrollback lines and misplaced prints
//! around height changes. Rendered through the reflowing
...
/// or OAuth messages) must be split into individual lines: ratatui's `Line`
/// drops embedded `\n`, and raw LF alone would not return to column 0 in raw
/// mode anyway.
#[tokio::test(start_paused = true)]
...
/// OAuth-required messages print the URL on its own line so the raw-mode
/// LF-without-CR staircase can't happen.
#[tokio::test(start_paused = true)]
...
/// gap+border+input+status plus a spinner row, so the decorative rows (gap,
/// spinner, border) are shed before squeezing the input or status rows.
#[tokio::test(start_paused = true)]
...
/// The /help box clamps itself to the terminal width so its rows don't wrap
/// (wrapped rows used to corrupt the scroll region on narrow terminals);
/// over-long rows are truncated at the box edge.
#[tokio::test(start_paused = true)]
