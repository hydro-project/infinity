    ...
    H: HttpClient + 'static,
{
    ...
    pub fn build_local(mut self) -> LocalAgentSystem<C, S, H> {
        // Every local system carries a launch registry; it stays empty (a
        // no-op in config resolution) unless the system is switched into
        // launcher mode with
        // [`with_thread_launcher`](LocalAgentSystem::with_thread_launcher).
...
/// How the threads of a built local system are created and attached to.
/// The default, [`Handles`], attaches to any thread ID on demand; switch to
/// [`Launcher`] with
/// [`with_thread_launcher`](LocalAgentSystem::with_thread_launcher) to create
/// threads through a [`ThreadBuilder`](super::local::ThreadBuilder) with their own
/// tools and prompt.
pub struct Handles;
...
///
/// `Mode` selects the thread API of the started system: with [`Handles`]
/// (the default), [`start`](Self::start) returns a
/// [`RunningSystem`](super::local::RunningSystem) whose
/// [`thread_handle`](super::local::RunningSystem::thread_handle) attaches to any
/// thread ID; after [`with_thread_launcher`](Self::with_thread_launcher),
/// [`start`](LocalAgentSystem::start) returns a
/// [`LaunchingSystem`](super::local::LaunchingSystem) whose threads are created
/// through a [`ThreadBuilder`](super::local::ThreadBuilder), each with its own tools
/// and prompt unioned onto the system-wide configuration.
pub struct LocalAgentSystem<C, S, H, Mode = Handles>
where
    C: ConversationStore,
    S: StateStore,
    H: HttpClient,
...
impl<C, S, H> LocalAgentSystem<C, S, H, Handles>
where
    C: ConversationStore + 'static,
    S: StateStore + 'static,
    H: HttpClient + 'static,
{
    /// Switch to launcher mode: [`start`](LocalAgentSystem::start) then
    /// returns a [`LaunchingSystem`](super::local::LaunchingSystem), whose threads
    /// are created through
    /// [`thread_builder`](super::local::LaunchingSystem::thread_builder) with their
    /// own tools and prompt (unioned onto the system-wide configuration) and
    /// whose [`thread_handle`](super::local::LaunchingSystem::thread_handle)
    /// re-attaches to existing threads only.
    pub fn with_thread_launcher(self) -> LocalAgentSystem<C, S, H, Launcher> {
        LocalAgentSystem {
            system: self.system,
