...
pub(crate) fn start_handle_system(
    tools: Vec<Box<dyn Tool<ChannelSender>>>,
) -> (
    RunningSystem<super::local::HandleSubscribeRequest>,
    MockModelController,
) {
...
/// Start a local system in launcher mode with the given system-wide tools
/// and prompt.
