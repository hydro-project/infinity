...
//!
//! Build a local system, start it, and launch configured threads through
...
use crate::system::builder::{AgentSystem, LocalAgentSystem};
...
impl<M: InputSender> Default for LaunchRegistry<M> {
    fn default() -> Self {
        Self {
            threads: Rc::new(RefCell::new(HashMap::new())),
        }
    }
}
...
pub(crate) struct UnionModelSource<C: ConversationStore, M: InputSender> {
    ...
    pub(crate) registry: LaunchRegistry<M>,
...
where
    ...
    H: HttpClient + 'static,
{
    /// Run the system with the built-in thread-builder API: create threads
    /// through [`LaunchingSystem::thread_builder`], and reattach to existing
    /// threads with [`LaunchingSystem::thread_handle`].
...
/// prompt unioned onto the system-wide configuration. New threads cannot be
/// created by attaching a handle: [`thread_handle`](Self::thread_handle) only
/// reattaches to a thread that was launched or already exists in the store.
    ...
    H: HttpClient + 'static,
{
    ...
    /// Wait for the next thread-driver lifecycle transition. See
    /// [`RunningSystem::next_lifecycle_event`].
    pub async fn next_lifecycle_event(&mut self) -> Option<ThreadLifecycleEvent> {
        self.running.next_lifecycle_event().await
    }
    ...
    async fn launcher_thread_handle_attaches_to_existing_threads_only() {
        ...
        local
            .run_until(async {
                ...
                assert!(
                    system
                        .thread_handle(&launched_id)
                        .await
                        .expect("check launched thread")
                        .is_some()
                );

                let history_thread = system.thread_builder().launch().await;
                let history_thread_id = history_thread.thread_id().to_owned();
                history_thread
                ...
                let mut attached = system
                    .thread_handle(&history_thread_id)
                    .await
