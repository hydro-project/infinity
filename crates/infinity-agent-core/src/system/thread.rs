    ...
    H: HttpClient + 'static,
{
    ...
    /// thread report, an incoming tool result, or user text. The echo itself
    /// is computed by the shared [`event_processor::input_echo`].
    fn emit_input_echo<O: ThreadObserver>(
        ...
        let event = match event_processor::input_echo(&self.history, input_msg) {
            Some(event_processor::InputEcho::SubscriptionEvent { name, text }) => {
                AgentEvent::SubscriptionEvent { name, text }
            }
            Some(event_processor::InputEcho::ToolResult { segments }) => {
                AgentEvent::ToolResult { segments }
            }
            Some(event_processor::InputEcho::UserInput { text }) => AgentEvent::UserInput { text },
            None => return,
        };
