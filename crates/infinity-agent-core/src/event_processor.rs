...
/// How an accepted ([`PrepareResult::Ready`]) input should be echoed to
/// display: a subscription event / thread report (named after the tool call
/// it answers), an incoming tool result, or plain user text.
///
/// The high-level step pipeline maps this to `AgentEvent`s, keeping the
/// subtle original-call lookup for synthetic events in exactly one place.
pub enum InputEcho {
    SubscriptionEvent {
        name: String,
        text: String,
...
/// Compute the [`InputEcho`] for an input that was just accepted into
    ...
    if let Some(synth) = input_msg.synthetic.as_ref() {
            ...
            && let ToolResultContent::Text(text) = res.content.first()
        {
                ...
                && let AssistantContent::ToolCall(c) = content.first()
            {
                ...
                return Some(InputEcho::SubscriptionEvent {
                    name,
                    text: text.text,
                });
            }
