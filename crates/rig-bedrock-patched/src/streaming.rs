...
impl CompletionModel {
    pub(crate) async fn stream(
        ...
        let mut prompt_with_history = request.messages()?;

        // Append a CachePoint to the last message so Bedrock can cache
        // everything up to (and including) that turn on subsequent calls.
        if let Some(last_msg) = prompt_with_history.last_mut() {
            last_msg.content.push(aws_bedrock::ContentBlock::CachePoint(
                aws_bedrock::CachePointBlock::builder()
                    .r#type(aws_bedrock::CachePointType::Default)
                    .build()
                    .expect("CachePointBlock build cannot fail"),
            ));
        }
