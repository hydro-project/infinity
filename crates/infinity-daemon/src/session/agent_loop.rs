...
mod tests {
    ...
    async fn spawn_test_agent_loop(
        ...
        (agent_tx, idle_rx, active_threads, shutdown_tx)
    }
    ...
    async fn routes_to_separate_thread_workers() {
        ...
        local
            .run_until(async {
                ...
                let (tx, mut idle_rx, _, _shutdown_tx) =
    ...
    async fn shutdown_persists_in_flight_tool_result() {
        ...
        local
            .run_until(async {
                ...
                let (tx, mut idle_rx, active_threads, shutdown_tx) =
                    spawn_test_agent_loop_with_tools(
                        "t1",
                        conv.clone(),
                        state,
                        model,
                ...
                shutdown_tx.send(()).expect("agent loop should be alive");
    ...
    async fn compaction_during_tool_call_corrupts_history() {
        ...
        local
            .run_until(async {
                ...
                let (tx, _idle_rx, _, _shutdown_tx) =
