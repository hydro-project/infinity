...
**MCP Tools:** MCP servers run in a Lambda proxy that spawns the MCP process (via npx/uvx), forwards requests, and returns results. The CDK `LambdaMCPToolSet` construct automatically creates `{name}_list_tools` and `{name}_invoke_tool` methods.
