...
**Launcher configuration** belongs to a root thread created through `thread_builder()`. It is suited to process-local jobs where the caller already has the tool instances and model source:
