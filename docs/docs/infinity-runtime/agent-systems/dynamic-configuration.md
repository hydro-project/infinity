...
For example, the following source gives every root conversation and its subagents the same tenant configuration, using `ConversationStore::get_ancestor_chain` to map a child back to its root before querying the tenant registry:
...
`.thread_config(...)` replaces `.tool(...)`, `.tools(...)`, `.extra_system_prompt(...)`, and `.rap_notifier(...)`. Put all four forms of configuration in the returned `ThreadConfig`. The runtime still adds its [built-in tools](../built-in/built-in-tools.md).

:::

### When Resolution Runs
A local system resolves `ThreadConfig` before a thread begins processing its first input, and the resolved value remains in use while that thread is active. If the thread becomes idle and later receives another message, the system will load it again and call `resolve` again.

This boundary is useful for resources that should exist only while a conversation is active. For example, a source can load a RAP manifest, start a tenant-specific tool server, and cache the resulting tools when `resolve` runs. An idle watcher can then release those resources after the system reports that every thread belonging to the root conversation has exited, and the next message will call `resolve` and reconstruct them. See [System and Thread Lifecycle](./overview.md#system-and-thread-lifecycle) for the local idle signals.

`ThreadConfig` itself is not stored in conversation history, so you should persist the information needed to reconstruct it, such as a tenant ID, repository path, or server configuration. A resumed thread can then receive the same configuration in a new process.

Resolution receives the ID of the thread that is about to run. For a root conversation this is the user-facing thread ID, but for `spawn_thread`, compaction, and subscription-event work, it can be a child ID. Map child IDs to the root when configuration belongs to the whole conversation, as in the example above, and keep the leaf ID when each subagent needs a distinct capability set. This choice affects both access control and agent behavior, so do not infer a tenant from the shape of a generated child ID.
...
The constructor accepts the source directly. A fixed deployment can pass `StaticModel`, while a multi-tenant deployment can read a persisted selection:
...
To switch models, update the record read by `ModelSelections`; the next completion round will resolve the new value. A completion that is already in progress continues with the model it started with. Keeping the selection in durable storage also means that a restarted process will make the same choice.

The same root-versus-leaf decision applies here: resolve through the root when subagents should inherit the conversation's model, and through the leaf when child threads have their own persisted selection. You should validate every stored selection against a catalog and define a fallback for models that have been removed.
...
Launch configuration is retained in process memory. Conversation history remains durable, but a process that resumes the thread cannot reconstruct its launch configuration from history. This means that you should use a `ThreadConfigSource` and a root-aware `ModelSource` when tools, instructions, or model choices must survive restarts, and [`thread_builder()`](./running-locally.md#configuring-a-new-thread) when creating and configuring a thread are one process-local operation.
