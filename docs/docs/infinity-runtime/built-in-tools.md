...
**`sleep_until_event_or_input()`**: Hibernate indefinitely. The tool is a no-op; the slice ends without scheduling anything, and the agent wakes when the next message arrives naturally (user input or subscription event). This is the tool agents use after setting up subscriptions when there's nothing else to do.

All sleep tools are interruptible. If a user message or subscription event arrives while the agent is sleeping, the runtime processes it immediately (sleep tools are *passive*, via `Tool::is_passive`, so pending sleep calls never hold back other work). The pending sleep result arrives later and is appended to history normally.
