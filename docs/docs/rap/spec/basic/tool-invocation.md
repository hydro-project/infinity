...
|---|---|---|---|
| `operation` | `string` | Yes | The tool operation name. MUST match a name defined in the tool's [toolset definition](/docs/rap/spec/basic/toolsets). |
| `arguments` | `object` | Yes | Tool arguments. MUST conform to the JSON Schema defined for this operation in the toolset. |
| `id` | `string` | Yes | Unique tool call identifier, typically generated by the LLM. MUST be included in the corresponding [tool result](/docs/rap/spec/basic/tool-result). |
...
| `callback_url` | `string` | Yes | The URL where the tool MUST POST results. See [Transport](/docs/rap/spec/basic/transport) for callback URL requirements. |
...
Detailed error information (invalid arguments, authorization failures, rate limits) SHOULD be communicated asynchronously via a [tool result](/docs/rap/spec/basic/tool-result) with an error description, not through HTTP status codes.
