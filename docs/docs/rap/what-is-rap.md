...
- **Agent hibernation.** When there's nothing to do, the agent process shuts down entirely. No idle compute. It restarts instantly when a message arrives, whether that's user input, a tool result, or an external event.

RAP is also fully compatible with MCP. Any MCP server works as a RAP tool through a backwards compatibility layer, so you keep the entire MCP ecosystem and gain async execution for the tools that need it.
...
Today's agent protocols assume tools are fast. Call a tool, get a result, move on. But real-world agent tasks involve waiting: for builds to finish, for humans to approve, for events to happen. Without a protocol-level answer, every team reinvents the same patterns of polling loops, webhook plumbing, state persistence, and wake-up scheduling.
...
- **[Infinity Code](/docs/infinity-code/overview)**: A hands-on way to try a RAP agent locally. Infinity Code is a coding agent built on the full Infinity stack.
...
- **[Infinity Runtime](/docs/infinity-runtime/overview)**: The reference RAP runtime, a Rust library (`infinity-agent-core`) that you can embed directly in your own process or deploy on AWS Lambda for production.
